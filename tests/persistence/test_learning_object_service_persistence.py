...
def test_update_knowledge_rolls_back_learning_object_and_version_on_audit_error(
    monkeypatch,
) -> None:
    monkeypatch.setenv(
        "DATABASE_URL",
...
def test_update_knowledge_does_not_commit_outside_transaction(
    monkeypatch,
) -> None:
    monkeypatch.setenv(
        "DATABASE_URL",
    ...
    try:
        ...
        # The service must not commit autonomously.
        # Rollback the current transaction and verify that
        # the update and its Version were not persisted.
        session.rollback()

        verification = SessionFactory()

        try:
            persisted = verification.get(
                LearningObjectModel,
                learning_object.id,
            )
            ...
            assert persisted is not None
            assert persisted.state == "Active"
            assert persisted.statement_text == (
                "Original knowledge statement"
            )

            versions = (
                verification.query(VersionModel)
                .filter(
                    VersionModel.learning_object_id
                    == learning_object.id,
                )
...
def test_update_knowledge_persists_matching_version_and_audit(
    monkeypatch,
) -> None:
    monkeypatch.setenv(
        "DATABASE_URL",
    ...
    try:
        ...
        assert versions[1].number == 2
        assert versions[1].snapshot["statement"]["text"] == (
            "Updated knowledge statement"
        )
        assert versions[1].snapshot["state"] == "Active"
        ...
        updated_audit = audits[-1]

        assert updated_audit.actor == "editor"
        assert updated_audit.metadata_ == {
            "version": 2,
        }

        assert updated_audit.metadata_["version"] == (
            versions[-1].number
        )
...
def test_get_history_preserves_all_versions_after_multiple_updates(
    monkeypatch,
) -> None:
    monkeypatch.setenv(
        "DATABASE_URL",
    ...
    try:
        ...
        assert [version.number for version in history] == [
            1,
            2,
            3,
        ]
...
def test_version_history_survives_new_session(
    monkeypatch,
) -> None:
    monkeypatch.setenv(
        "DATABASE_URL",
    ...
    try:
        ...
        assert history[0].learning_object_id == learning_object_id
        assert history[1].learning_object_id == learning_object_id
        ...
        # Historical Version 1 must remain unchanged after Version 2.
        assert (
            history[0].snapshot["statement"]["text"]
            != history[1].snapshot["statement"]["text"]
        )
